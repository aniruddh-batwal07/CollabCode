...
import { createSnapshot, getSnapshots, getSnapshotById } from "./persistence/snapshotRepository";
...
setInterval(async () => {
  try {
    for (const [roomId, state] of roomDocs.entries()) {
      await createSnapshot(roomId, state);
    }
...
io.on("connection", (socket) => {
  ...
  socket.on("join-room", async ({ roomId, username }: { roomId: string; username: string }) => {
    ...
    let roomState: Uint8Array | undefined | null = roomDocs.get(roomId);

    if (!roomState) {
      const dbState = await getDocument(roomId);

      // Re-check after async DB read — a concurrent yjs-update may have
      // populated roomDocs while we were awaiting.
      roomState = roomDocs.get(roomId);

      if (!roomState && dbState) {
        roomDocs.set(roomId, dbState);
        roomState = dbState;
      }
    }

    if (roomState) {
      socket.emit("document-sync", Array.from(roomState));
    }
  });
  ...
  socket.on("yjs-update", async ({ roomId, update }: { roomId: string; update: number[] }) => {
    ...
    roomDocs.set(
      roomId,
      existing ? Y.mergeUpdates([existing, incoming]) : incoming
    );

    await saveDocument(roomId, roomDocs.get(roomId)!);

    socket.to(roomId).emit("yjs-update", update);
  });

  socket.on("get-snapshots", async (roomId: string) => {
    const snapshots = await getSnapshots(roomId);
    socket.emit("snapshots-list", snapshots);
  });

  socket.on("restore-snapshot", async ({ roomId, snapshotId }: { roomId: string; snapshotId: number }) => {
    const snapshot = await getSnapshotById(snapshotId);
    if (!snapshot) return;

    roomDocs.set(roomId, snapshot);
    await saveDocument(roomId, snapshot);
    io.to(roomId).emit("restore-sync", Array.from(snapshot));
  });
